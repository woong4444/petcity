document.addEventListener("DOMContentLoaded", function () {
    ...
    const form = document.querySelector("#mainBannerEditForm");
    ...
    const imageChangeOptions = Array.from(
        document.querySelectorAll(
            'input[name="imageChangeType"]',
        ),
    );

    const imageKeepArea = document.querySelector("#imageKeepArea");
    ...
    imageChangeOptions.forEach(function (option) {
    ...
    if (imageUrlInput !== null) {
        imageUrlInput.addEventListener("input", function () {
            ...
            if (imageUrl.length === 0) {
                showPreviewImage(currentImageUrl);
                return;
            }
    ...
    function updateImageChangeScreen() {
        const imageChangeType =
            getImageChangeType();

        if (imageKeepArea !== null) {
            imageKeepArea.hidden =
                imageChangeType !== "KEEP";
        }

        if (imageFileArea !== null) {
            imageFileArea.hidden =
                imageChangeType !== "FILE";
        }
